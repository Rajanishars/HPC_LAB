...
void* producer(void* arg) {
    ...
    while (1) {
        ...
        if (produced >= TOTAL_TASKS) {
            sem_post(&mutex);
            sem_post(&empty); // Give back the slot
            break;
        }
...
void* consumer(void* arg) {
    while (1) {
        ...
        if (consumed >= TOTAL_TASKS) {
            sem_post(&mutex);
            sem_post(&full); // Wake up the next stuck consumer
            break;
        }
       ...
       // fprintf(fp, "%d %c %d = %.2lf\n", t.a, t.op, t.b, result);
        printf("\n--------Arithmetic Operation performs using PThread Synchronization--------\n");
...
int main() {
    ...
    printf("Results written to operation.txt\n"); // Added \n for instant printing
    return 0;
}
